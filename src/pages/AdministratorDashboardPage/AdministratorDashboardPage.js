...
export default class AdministratorDashboardPage extends Component {
              ...
              {this.state.orders.map(order => {
                return(
                  ...
                  <tr className='text-secondary'>
                    ...
                    <td>
                      <select class={`form-select text-light ${order.status === "PENDING" ? 'bg-secondary' : ""} ${order.status === "ACCEPTED" ? 'bg-info' : ""} ${order.status === "REJECTED" ? 'bg-danger' : ""} ${order.status === "SHIPPED" ? 'bg-success' : ""}`} aria-label="Default select example" onChange={async (e) => this.statusChanged(order.userOrderId, e)}>
  ...
  async statusChanged(orderId, e) {

    await api.apiToken("api/admin/change/status/order/" + orderId, "put", {
