...
export default class AdministratorPetsPage extends Component {
    ...
    async statusChanged(id, e) {
        await api.apiToken("api/admin/edit/status/pet/" + id, "put", {
                        ...
                        this.state.pets.map(pet => {
                            return(
                                <tr>
                                    ...
                                    ${pet.status === "SHIPPING" ? 'bg-warning' : ""} 
                                    ${pet.status === "SOLD" ? 'bg-danger' : ""} 
                                    ${pet.status === "ON_SALE" ? 'bg-success' : ""}`}
                                     aria-label="Default select example" 
                                     onChange={async (e) => this.statusChanged(pet.petId, e)}
                                     >
                        <option value="ON_SALE" selected={pet.status === "ON_SALE" ? true : false}>Na prodaju</option>
                        <option value="SOLD" selected={pet.status === "SOLD" ? true : false}>Prodano</option>
                        <option value="SHIPPING" selected={pet.status === "SHIPPING" ? true : false}>Dostava</option>
                      </select>
                                    </td>
                                    <td>
